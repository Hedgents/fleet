...
Current target through-cycle return on a $150,000 equal-weight portfolio: **8–11 % annualised**. Live mainnet operation since 2026-05-09. **Two of the three strategies are now executing real positions on Solana mainnet:** stable-yield supplies ~$55 USDC to Kamino at the live supply rate, and multiply runs a 35 %-LTV leveraged jitoSOL position (~$14 collateral against ~$5 SOL debt) via two committed lever-up rounds. Total on-chain deployed: **~$64**. A public reference deployment runs on a Hetzner ARM VM in Falkenstein, DE, at [dashboard.hedgents.com](https://dashboard.hedgents.com). This document describes what we have built, what risks an operator is taking, and what we have not yet done.
...
- **Stable-yield live since 2026-05-12.** Daemon supplied $5 USDC into Kamino's main USDC reserve, then $50 follow-on after hardening the second-deposit code path. **~$55 currently earning the live Kamino USDC supply APR (~9 %).** Solscan: [`4FH94y…brb6J`](https://solscan.io/tx/4FH94y6jnbfZxetjXQgKaqTdX7sPMk3pUy2PJEPhK2m3MDfLRvrNDEQ37KmWPW56SVX8SsRHbrewsQZXrQ1brb6J), [`4R6pJe…1EuN`](https://solscan.io/tx/4R6pJeH5uo8ScWfiVori5GUu9NY3S8rncWipEfBKeywqfhxjRkwxUwzq9ywVRzdoqCqrmYtRv5JbHcKecXXi1EuN). Recoverable via `WithdrawStableLend`.
- **Multiply live since 2026-05-14.** Multiply daemon bootstrapped its own isolated Kamino obligation (cross-collateralisation-safe vs stable-yield), staked SOL → jitoSOL via Jito, deposited as collateral, then ran two atomic lever-up rounds via klend's v2 handlers (CPI farm-refresh, ALT-compressed bundle within Solana's 1232-byte limit). Final position: 119M jitoSOL cTokens (~$14) collateral, 53M SOL lamports (~$5) borrowed, **~35 % LTV** against an 80 % liquidation threshold. Solscan: [seed](https://solscan.io/tx/5qQd5kYti7UgXfpyhg9bzCZKCUrieu6nXEiu41ZbovDzTL185gQytitSaxFQaQjJZu3BF58HhFD8k2jriFszce7x), [round 1](https://solscan.io/tx/bZ4tzz51gkeMTfJn9R6bx8nq3YqG8i3yxgoM8kLCB2Lyh9Ff89EXf7yW6Wc6BrnnCiT1kXsqdygf7shnH7u4scN), [round 2](https://solscan.io/tx/4aU8sqWf4ioF6yLHJ5RGB93J9hFf5AogEbYv3AutXLxS83QYZWVJdLtRTAvJXEcoKA4zheEU2GgbWpS1oCmVdWi8).
- **Pre-pilot bug discipline.** Twenty patched releases (`fleet-v0.1.4` → `v0.1.24`) shipped over 36 hours against real mainnet broadcasts before any institutional pilot — Kamino obligation idempotency, ixn-ordering for klend's `check_refresh_ixs!` contract, jitoSOL exchange-rate math, klend v2 handler migration to inline farm-refresh CPIs, Solana Address Lookup Table compression for the leverage bundle, mesh dispatch payload-type guard, and a verified-against-source audit of the obligation account-layout decoder (`docs/kamino-loader-audit-2026-05-14.md`). Each release has unit-test coverage, CI-signed artifacts, and the same install path institutional operators will use.
...
- **Mainnet position at full runbook size for the third strategy**: $200 hedged JLP. The current $55 stable-yield + $14 multiply positions exercise the lending and leveraged-staking paths end-to-end on-chain; hedged-JLP requires Jupiter Perps integration which is implemented but not yet broadcast live.
- **Scaling beyond tiny-position sizes.** Today's positions are bring-up tests (sub-$100). Institutional sizes ($50K – $5M per strategy) need a 24h+ watch on the current sizes plus a multi-wallet sharding roll-out plan (see Privacy roadmap below).
- **Privacy roadmap.** Today's positions are pseudonymously on-chain but trivially correlatable via the single fleet wallet. Production deployments will use multi-wallet sharding (split each strategy across N operator-owned wallets) and, in v1.0, Light Protocol compressed-account encryption so position state is private to the operator.
