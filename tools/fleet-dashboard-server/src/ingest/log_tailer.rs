...
//!
//! Resilience:
//! - On startup, every existing `*.log` file is seeked to end-of-file
//!   (`tail -F`-style) — historic lines have already been ingested in
//!   previous runs; re-reading 200MB of journals on every restart pegs
//!   sqlite, starves the notify watcher of fresh events, and freezes the
//!   `/daemons` heartbeat readout for many minutes. New content appended
//!   after startup is ingested as soon as the watcher fires.
...
pub async fn run(dir: PathBuf, tx: mpsc::Sender<RawLogLine>) -> Result<()> {
    ...
    // Initial sweep: seed offsets at end-of-file (`tail -F`-style). Historic
    // BEACONs have already been ingested on prior runs; re-reading the
    // whole on-disk journal on every restart starves the notify watcher
    // and freezes `/daemons` heartbeats for minutes. New content appended
    // after startup is ingested as soon as `notify` fires.
