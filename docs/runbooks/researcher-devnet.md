...
Note: lending and JLP watchers omitted from the boot example since their underlying decoders are 0-stubbed in v0. Adding them is harmless (they tick + log) but won't emit until the decoders land. (Perp-funding watcher was removed 2026-05-06 after Drift Protocol's April hack; will be reintroduced as a Jupiter Perps borrow-rate watcher when hedgedjlp ships.)
...
- `<watcher_name> first observation seeded` for stateful watchers (price, jlp_yield)
...
| Watchers boot but no signals fire | All watchers below threshold (normal) OR underlying decoder 0-stubbed (M3 lending APR, M7 JLP yield) | Check the M-numbered comments in each watcher; M5 price + M6 stable_peg are the only fully-real watchers in v0 |
...
- Perp borrow-rate watcher (originally M4 against Drift; removed 2026-05-06 after Drift hack; will be reintroduced as a Jupiter Perps borrow-rate watcher when hedgedjlp ships)
