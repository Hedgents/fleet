...
/// Minimal application state used by the read-only Pyth handler.
///
/// Lifted from the monolith's `AppState` but stripped to just the fields the
/// price endpoint needs — no wallet, no pairing, no signing surface. The
/// fleet-wide pairing/identity layer lives in the runtime crate's `pairing`
/// module and will be wired in by the streams subscriber, not by this handler.
#[derive(Clone)]
pub struct AppState {
    pub rpc: RpcContext,
