...
struct RunArgs {
    ...
    /// `onyc-role.key` (32 raw bytes) from this directory.
    ...
    /// a 5-minute pause on new AssignOnyc (returned with error_code=4).
    ...
    /// M11 auto-mode: when true, AssignOnyc envelopes whose sender
...
impl Daemon for Multiply {
    ...
    async fn run(self: Box<Self>) -> Result<()> {
        ...
        let monitor_ctx = liq_monitor::LiqMonitorCtx {
            ...
            lending_market: zerox1_defi_protocols::constants::KAMINO_ONYC_MARKET,
            ...
            // v0.4.29c: NAV-aware controller state. Empty at boot; each
            // liq_monitor::tick pushes the current ONyc NAV observation.
            nav_history: Arc::new(tokio::sync::Mutex::new(
                crate::nav_controller::NavHistory::new(),
            )),
        };
        ...
        let dispatch_ctx = dispatch::DispatchCtx {
            ...
            // capital arrives via AssignOnyc.usdc_lamports. Lite
...
fn run_daemon(args: RunArgs) -> Result<()> {
    ...
    rt.block_on(async move {
        ...
        let role_identity = load_role_identity(&secrets, Role::Onyc, "onyc-role.key")
            .await
