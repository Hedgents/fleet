//! Manual-approval queue for AssignOnyc.
//!
...
//! the (conversation_id, AssignOnyc, sender_pubkey) tuple, emits
...
pub struct ApprovalQueue {
    ...
    pending: Mutex<HashMap<[u8; 16], (AssignOnyc, Instant, [u8; 32])>>,
}
...
impl ApprovalQueue {
    ...
    pub fn enqueue(&self, conv: [u8; 16], assign: AssignOnyc, sender: [u8; 32]) -> bool {
...
/// Parallel approval queue for `WithdrawOnyc` payloads. Same shape as
...
pub enum WithdrawApproveResult {
    Approved(WithdrawOnyc),
    NotFound,
    SenderMismatch { expected: [u8; 32], got: [u8; 32] },
}

pub struct WithdrawApprovalQueue {
    pending: Mutex<HashMap<[u8; 16], (WithdrawOnyc, Instant, [u8; 32])>>,
}
...
impl WithdrawApprovalQueue {
    ...
    /// Enqueue a `WithdrawOnyc`. Returns true on success, false if
    /// the queue is at capacity (same `MAX_PENDING` as the Assign queue).
    pub fn enqueue(&self, conv: [u8; 16], withdraw: WithdrawOnyc, sender: [u8; 32]) -> bool {
        let mut p = self.pending.lock().unwrap();
        p.retain(|_, (_, t, _)| t.elapsed() < APPROVAL_TTL);
        if p.len() >= MAX_PENDING {
            return false;
