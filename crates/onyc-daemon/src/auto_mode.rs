...
//!   * AssignOnyc — sized by `target_ltv_bps` (no USD field).
//!     Auto-mode gate is `target_ltv_bps <= auto_max_target_ltv_bps`.
//!   * WithdrawOnyc — full-unwind only (no amount field). Treated as
...
use zerox1_protocol::fleet::onyc::AssignOnyc;
...
/// Decide the dispatch path for an AssignOnyc envelope. Public so
...
/// Auto-mode gate for AssignOnyc. Decides whether to bypass the
...
mod tests {
    ...
    fn under_caps_accepts() {
        ...
        let st = AutoModeState::new();
        match gate_assign_multiply(&cfg, &st, &assign(6000, 50), 1_000) {
            GateDecision::Accept { label, .. } => assert_eq!(label, "AssignOnyc"),
    ...
    fn full_deleverage_bypasses_cumulative_cap() {
        ...
        match gate_assign_multiply(&cfg, &st, &assign(0, 50), 1_001) {
            GateDecision::Accept { label, .. } => {
                assert_eq!(label, "AssignOnyc(full-deleverage)");
            }
...
mod dispatch_path_tests {
    ...
    //!     reachable for AssignOnyc but pinned for future-proofing)
    ...
    fn assign(target_ltv: u16) -> AssignOnyc {
        AssignOnyc {
            vault: [0; 32],
            target_ltv_bps: target_ltv,
            max_slippage_bps: 50,
    ...
    fn auto_accept_under_caps() {
        ...
        match decide_assign_multiply(&cfg, &st, Some(ORCH), ORCH, &assign(6000), 1_000) {
            DispatchPath::AutoExecute { label, .. } => assert_eq!(label, "AssignOnyc"),
    ...
    fn auto_accept_falls_through_above_single_cap_is_structurally_unreachable() {
        // Multiply has no USD field on AssignOnyc — the single-action
        ...
        //
        // If a future protocol revision adds a USD field to AssignOnyc,
