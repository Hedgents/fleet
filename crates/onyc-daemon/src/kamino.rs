...
//!      pulled via `solana account <KAMINO_ONYC_USDC_RESERVE>` decoded
...
use zerox1_defi_protocols::{
    constants::{
        ASSOCIATED_TOKEN_PROGRAM_ID, KAMINO_LEND_PROGRAM_ID, KAMINO_ONYC_MARKET,
        KAMINO_ONYC_USDC_RESERVE, SPL_STAKE_POOL_PROGRAM_ID, SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID,
        USDC_MINT,
    },
    protocols::kamino::{
...
pub async fn build_supply_ixns(
    ...
    let mut ixs = deposit_ix(
        ...
        &reserve,
        amount_lamports,
        crate::caps::ONYC_OBLIGATION_SEED,
        &obligation_reserves,
    )?;
...
fn usdc_reserve_accounts() -> ReserveAccounts {
    ReserveAccounts {
        reserve: KAMINO_ONYC_USDC_RESERVE,
        lending_market: KAMINO_ONYC_MARKET,
        lending_market_authority: derive_lending_market_authority(&KAMINO_ONYC_MARKET),
