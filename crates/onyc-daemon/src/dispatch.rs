//! Inbox dispatch — decode AssignOnyc, validate against caps,
//! call leverage::run_or_simulate, build ReportOnyc, sign + send.
...
use zerox1_protocol::fleet::onyc::{
    AssignOnyc, ReportOnyc, ReportOnycWithdraw, WithdrawOnyc,
};
...
/// arrives from the trusted riskwatcher. Multiply refuses new AssignOnyc
/// for this many seconds, then auto-clears on the next inbound Assign.
pub const PAUSE_DURATION_SECS: u64 = 300;

/// error_code returned in ReportOnyc when an Assign is rejected because
...
pub struct DispatchCtx {
    ...
    /// v0.3.0: parallel approval queue for WithdrawOnyc. Same TTL +
    ...
    /// riskwatcher M7: when `Some(t)`, multiply refuses new AssignOnyc
    ...
    /// (delivered via `AssignOnyc.usdc_lamports`) into native SOL
...
/// AssignOnyc (the only Assign type this daemon owns). Defence-in-depth
...
fn payload_is_for_this_daemon(env: &Envelope) -> bool {
    match env.msg_type {
        MsgType::Assign => ciborium::de::from_reader::<AssignOnyc, _>(&env.payload[..]).is_ok(),
        MsgType::WithdrawOnyc => {
            ciborium::de::from_reader::<WithdrawOnyc, _>(&env.payload[..]).is_ok()
        }
        _ => true,
    }
}
...
/// AssignOnyc CBOR payload.
pub async fn run(mut handle: NodeHandle, ctx: DispatchCtx) -> Result<()> {
    // rc28: dedupe Assign/WithdrawOnyc by conv_id; see hedgedjlp.
    ...
    while let Some(env) = handle.recv().await {
        ...
        match env.msg_type {
            ...
            MsgType::Approve => {
                ...
                match ctx.approval_queue.approve(conv, env.sender) {
                    crate::approval::ApproveResult::Approved(payload) => {
                        info!(?conv, "Approve received — executing queued AssignOnyc");
                    ...
                    crate::approval::ApproveResult::NotFound => {
                        ...
                        // queued WithdrawOnyc. Probe the withdraw queue
                        ...
                        match ctx.withdraw_approval_queue.approve(conv, env.sender) {
                            crate::approval::WithdrawApproveResult::Approved(w_payload) => {
                                info!(
                                    ?conv,
                                    "Approve received — executing queued WithdrawOnyc"
                                );
                                if let Err(e) = caps::validate_withdraw_onyc(&w_payload) {
                                    warn!(
                                        ?e,
                                        ?conv,
                                        "post-approval withdraw cap re-validation failed"
                                    );
                                    let report = ReportOnycWithdraw {
                                        header: ReportHeader::err(conv, 2),
            ...
            MsgType::WithdrawOnyc => {
                ...
                if !sender_is_authorised(ctx.orchestrator_agent_id, env.sender, "WithdrawOnyc")
                {
                    continue;
                }
                if !dedupe.record(conv) {
                    info!(
                        ?conv,
                        sender = %hex::encode(env.sender),
                        "rc28: duplicate WithdrawOnyc dropped (first delivery already processed)"
                    );
                    continue;
                }
                if is_paused(&ctx) {
                    warn!(
                        ?conv,
                        "WithdrawOnyc rejected — paused by riskwatcher veto"
                    );
                    let report = ReportOnycWithdraw {
                        header: ReportHeader::err(conv, ERR_PAUSED_BY_RISKWATCHER),
                        final_usdc_lamports: 0,
            ...
            MsgType::MarketSignal => {
                ...
                // strategy-specific reactions (e.g. pause new leverage
                // on Pyth SOL PriceMovedBps Warning+).
                info!(
                    sender = %hex::encode(env.sender),
...
/// Bilateral send of a `ReportOnycWithdraw` envelope back to the
/// orchestrator that issued the `WithdrawOnyc`. Mirrors
...
async fn send_report_withdraw(
    ...
    ciborium::ser::into_writer(&report, &mut payload)
        .context("serialize ReportOnycWithdraw")?;
...
async fn handle_assign(
    ...
    let payload: AssignOnyc = ciborium::de::from_reader(&env.payload[..])
        .context("decode AssignOnyc CBOR payload")?;
    ...
    if payload.usdc_lamports > 0 {
        let Some(jup) = ctx.jupiter.as_ref() else {
            bail!(
                "AssignOnyc.usdc_lamports={} but multiply-daemon was started \
    ...
    if ctx.require_approval {
        info!(?conv, "AssignOnyc queued — awaiting Approve");
        let added = ctx
            .approval_queue
...
/// Handle an inbound WithdrawOnyc envelope. Decodes, cap-validates,
...
async fn handle_withdraw(
    ...
    let payload: WithdrawOnyc = ciborium::de::from_reader(&env.payload[..])
        .context("decode WithdrawOnyc CBOR payload")?;

    info!(
        max_slippage_bps = payload.max_slippage_bps,
        deadline_unix = payload.deadline_unix,
        "WithdrawOnyc received"
    );

    caps::validate_withdraw_onyc(&payload).context("withdraw cap validation")?;
    ...
    if ctx.require_approval {
        let conv = env.conversation_id;
        info!(?conv, "WithdrawOnyc queued — awaiting Approve");
        let added = ctx
            .withdraw_approval_queue
            .enqueue(conv, payload.clone(), env.sender);
        if !added {
            return Err(anyhow!(
                "withdraw approval queue full (cap 64); rejecting WithdrawOnyc"
            ));
        }
        if let Err(e) = emit_needs_approval(handle, ctx, env).await {
            warn!(
                ?e,
                ?conv,
                "failed to emit NeedsApproval Escalate for WithdrawOnyc; queued anyway"
            );
        }
        return Ok(ReportOnycWithdraw {
            header: ReportHeader::ok(conv),
...
async fn send_report(
    ...
    let mut payload = Vec::new();
    ciborium::ser::into_writer(&report, &mut payload).context("serialize ReportOnyc")?;
    ...
    // can populate its observed-positions registry. ReportOnyc is
...
mod payload_filter_tests {
    ...
    //! whose payload isn't AssignOnyc.
    ...
    fn stable_lend_withdraw_on_multiply_slot_is_dropped() {
        ...
        // WithdrawOnyc slot must be filtered out so the multiply
        ...
        // WithdrawOnyc has 3 fields where one is a 32-byte array.
