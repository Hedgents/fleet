...
//!
//! Stable-yield owns two payload shapes:
//!   * AssignStableLend — `usdc_lamports` is the size metric (6 decimals).
//!   * WithdrawStableLend — `usdc_lamports`. `u64::MAX` sentinel means
//!     "withdraw all"; treated as exceeding the single-action cap (so
//!     full-withdraws fall through to manual approval).
//!
//! 24h tracking is in-memory only (matches the existing telemetry-state
...
/// Returns true iff `sender` is the configured orchestrator. Auto-mode
/// requires a configured orchestrator to be meaningful; devnet sandbox
/// (orchestrator=None) deliberately can't auto-accept.
...
/// by both AssignStableLend and WithdrawStableLend after their
/// payload-specific sizing.
fn gate_common(
    cfg: &AutoModeConfig,
    state: &AutoModeState,
    usd_lamports: u64,
...
/// Decide the dispatch path for an AssignStableLend envelope.
pub fn decide_assign_stable_lend(
    cfg: &AutoModeConfig,
    state: &AutoModeState,
    ...
    gate_common(cfg, state, payload.usdc_lamports, "AssignStableLend", now)
}

/// Decide the dispatch path for a WithdrawStableLend envelope.
///
/// `u64::MAX` (full-withdraw sentinel) is treated as exceeding the
/// single-action cap — full withdrawals always require manual approval
/// because their realised USD value depends on a chain read the gate
/// doesn't perform.
pub fn decide_withdraw_stable_lend(
    cfg: &AutoModeConfig,
    state: &AutoModeState,
    orchestrator: Option<[u8; 32]>,
    sender: [u8; 32],
    ...
    if payload.usdc_lamports == u64::MAX {
        return DispatchPath::Queue {
            cap: "single_action_usd",
...
mod tests {
    ...
    fn assign(usdc: u64) -> AssignStableLend {
        AssignStableLend {
            market: [0; 32],
            reserve: [0; 32],
            usdc_lamports: usdc,
    ...
    fn withdraw(usdc: u64) -> WithdrawStableLend {
        WithdrawStableLend {
            market: [0; 32],
            reserve: [0; 32],
            usdc_lamports: usdc,
    ...
    fn auto_accept_under_caps() {
        ...
        // $40 is under the $50 default single-action cap.
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), ORCH, &assign(40_000_000), 1_000) {
            DispatchPath::AutoExecute { label, .. } => assert_eq!(label, "AssignStableLend"),
    ...
    fn auto_accept_falls_through_above_single_cap() {
        ...
        let st = AutoModeState::new();
        // $60 > $50 default
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), ORCH, &assign(60_000_000), 1_000) {
    ...
    fn auto_accept_falls_through_above_cumulative_cap() {
        let mut cfg = cfg_on();
        cfg.cooldown_secs = 0;
        // Cumulative cap = $100; one $50 prior accept, then this $50 would
        // sum to $100 (≤ cap, ok), but the next $50 would exceed.
        cfg.max_cumulative_24h_usd_lamports = 100_000_000;
        let st = AutoModeState::new();
        st.record_at(1_000, 50_000_000);
        st.record_at(1_001, 50_000_000);
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), ORCH, &assign(10_000_000), 1_002) {
    ...
    fn auto_accept_cooldown_falls_through() {
        ...
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), ORCH, &assign(10_000_000), 1_030) {
    ...
    fn auto_accept_rejects_non_orchestrator_sender() {
        let cfg = cfg_on();
        let st = AutoModeState::new();
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), OTHER, &assign(10_000_000), 1_000) {
    ...
    fn auto_accept_off_by_default() {
        ...
        let st = AutoModeState::new();
        match decide_assign_stable_lend(&cfg, &st, Some(ORCH), ORCH, &assign(10_000_000), 1_000) {
    ...
    fn auto_accept_with_no_orchestrator_configured_falls_through() {
        let cfg = cfg_on();
        let st = AutoModeState::new();
        match decide_assign_stable_lend(&cfg, &st, None, ORCH, &assign(10_000_000), 1_000) {
            DispatchPath::Queue { cap, .. } => assert_eq!(cap, "non-orchestrator-sender"),
            other => panic!("expected Queue, got {other:?}"),
        }
    }
    ...
    #[test]
    fn withdraw_full_sentinel_falls_through() {
        let cfg = cfg_on();
        let st = AutoModeState::new();
        match decide_withdraw_stable_lend(&cfg, &st, Some(ORCH), ORCH, &withdraw(u64::MAX), 1_000) {
            DispatchPath::Queue { cap, .. } => assert_eq!(cap, "single_action_usd"),
            other => panic!("expected Queue (full-withdraw must be manual), got {other:?}"),
        }
    }
