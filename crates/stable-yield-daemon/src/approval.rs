...
//! implementation can hold either AssignStableLend or WithdrawStableLend
//! (M10). Each queue instance only holds one payload type — the daemon
...
pub type AssignApprovalQueue = ApprovalQueue<AssignStableLend>;
pub type WithdrawApprovalQueue = ApprovalQueue<WithdrawStableLend>;
...
mod tests {
    ...
    fn withdraw() -> WithdrawStableLend {
        WithdrawStableLend {
            market: [0; 32],
            reserve: [0; 32],
            usdc_lamports: 25_000_000,
            deadline_unix: 0,
        }
    }
    ...
    fn enqueue_then_approve_with_matching_sender() {
        ...
        match q.approve(conv, sender) {
            ApproveResult::Approved(a) => assert_eq!(a.usdc_lamports, 50_000_000),
    ...
    fn withdraw_queue_round_trip() {
        ...
        match q.approve(conv, sender) {
            ApproveResult::Approved(w) => assert_eq!(w.usdc_lamports, 25_000_000),
            _ => panic!("expected Approved"),
        }
    }

    #[test]
    fn contains_only_matches_on_sender() {
        let q: WithdrawApprovalQueue = ApprovalQueue::new();
        let conv = [33u8; 16];
        let owner = [11u8; 32];
        let stranger = [12u8; 32];
        q.enqueue(conv, withdraw(), owner);
        assert!(q.contains(conv, owner));
        assert!(!q.contains(conv, stranger));
        assert!(!q.contains([99u8; 16], owner));
    }
}
