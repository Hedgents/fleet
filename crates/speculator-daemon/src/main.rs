...
//! instance and joins the 0x01 mesh as a long-lived role identity.
//! Speculator is a *signing* daemon: it keeps the existing
//! `Wallet::load` / `SigningWhitelist` plumbing and augments it with
//! the embedded mesh node.
//!
//! Runtime: `MultiThread { workers: 2 }`. A current-thread Tokio
//! runtime fights with libp2p's event loop on the pinned core; two
//! workers — one for the trade-execution hot path, one for libp2p /
//! housekeeping — is the practical compromise. Real core pinning is
//! a strategy-plan concern; the `pin_to_core` stub is preserved.
//!
//! TODO(strategy plan): wire the trade-execution dispatcher into
//! `handle_inbox`, gated by `quote_ttl_ms`.
...
struct Args {
    ...
    /// Path to the Solana keypair used to sign Jupiter v6 swap ixns.
    #[arg(long, env = "ZX_WALLET")]
    wallet: PathBuf,

    /// Optional core to pin the runtime to. Stub-only today (logs a warn).
    #[arg(long, env = "ZX_PIN_CORE")]
    pin_core: Option<usize>,

    /// Quote freshness budget, milliseconds. Used by the strategy plan's
    /// trade-execution hot path; logged at boot for visibility.
    #[arg(long, env = "ZX_QUOTE_TTL_MS", default_value_t = 750)]
    quote_ttl_ms: u64,

    /// Directory holding role secret files. The daemon reads
    /// `speculator-role.key` (32 raw bytes) from this directory.
    ...
    /// the old health bind (9305), now multiaddr-shaped.
    #[arg(long, env = "ZX_LISTEN", default_value = "/ip4/0.0.0.0/tcp/9305")]
    listen: String,
...
fn build_node_config(args: &Args, role_id: &RoleIdentity) -> Result<NodeConfig> {
    ...
    argv.push(format!("speculator-{}", args.fleet_id));
...
/// FleetIntent from the orchestrator, fan out signed swap tx receipts).
...
fn main() -> Result<()> {
    ...
    // Existing speculator boot logic — Wallet/whitelist are kept and
    ...
    rt.block_on(async move {
        ...
        let role_identity =
            load_role_identity(&secrets, Role::Speculator, "speculator-role.key")
                .await
                .context("loading speculator role key")?;
